...
def _read_csv(path: Path) -> tuple[list[str], list[dict]]:
    ...
    with path.open(newline="", encoding="utf-8") as fh:
        reader = csv.DictReader(fh) # why csv.DictReader and not just csv.reader? because we want to access columns by name, not by index. csv.reader gives you rows as plain lists — ["Acetone", "850"]. You'd have to remember that index 0 is the name and index 1 is the quantity. Fragile. csv.DictReader gives you rows as dicts — {"Name": "Acetone", "Quantity, kg": "850"}. You access columns by name. Much safer, especially when column order might vary.#
...
def process_file(input_path: Path, output_dir: Path) -> Path:
    ...
    unique_names = list(dict.fromkeys(names))  # deduplicate, preserve order # list(set(names)) would also deduplicate but scrambles the order. dict.fromkeys() preserves insertion order while removing duplicates — a Python dict can't have duplicate keys, so this is an elegant trick. Acetone appears in all three batch files — we only want to call the API for it once.
